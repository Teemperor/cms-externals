      SUBROUTINE ds_s2(P,I3,I4,H1,H2,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     BEGIN CODE
C     ----------
      IF(I3.EQ.1 .AND. I4.EQ.3)CALL ds_ds_s2(P,H1,H2,ANS)

      END

      SUBROUTINE ds_ds_s2(P,H1,H2,ANS)
C     
...
C     
C     Process: d s > d s x QED<=99 NPqq<=99 NPll<=99 QCD<=99 NPgg<=99
C      NPvv<=99 @1
...
C     ----------
      ...
      ANS = 0D0
      DO IHEL=1,NCOMB
            T=MATRIX_ds_ds_s2(P,H1,H2 ,NHEL(1,IHEL),JC(1))
            ANS=ANS+T
      ENDDO
      ANS=ANS/DBLE(IDEN)
      END
      ...
      REAL*8 FUNCTION MATRIX_ds_ds_s2(P,H1,H2,NHEL,IC)
C     
C     Generated by MadGraph5_aMC@NLO v. 2.4.3, 2016-08-01
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
