C  MB. corrected 23.12.2016
      SUBROUTINE gcx_s2(P,I3,I4,H1,H2,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     BEGIN CODE
C     ----------
      IF(I3.EQ.21 .AND. I4.EQ.-4) CALL gcx_gcx_s2(P,H1,H2,ANS)
      IF(I3.EQ.21 .AND. I4.EQ.-3) CALL gcx_gcx_s2(P,H1,H2,ANS) 
      IF(I3.EQ.-4 .AND. I4.EQ.21) CALL gcx_gcx_s2(PP,H1,H2,ANS)
      IF(I3.EQ.-3 .AND. I4.EQ.21) CALL gcx_gcx_s2(PP,H1,H2,ANS) 
    
      END
      
      SUBROUTINE gcx_gcx_s2(P,H1,H2,ANS)
C     
...
C     
C     Process: g c~ > g c~ x QED<=99 NPqq<=99 NPll<=99 QCD<=99
C      NPgg<=99 NPVV<=99 @1
C     *   Decay: x > ta+ ta- WEIGHTED<=1
C     Process: g s~ > g s~ x QED<=99 NPqq<=99 NPll<=99 QCD<=99
C      NPgg<=99 NPVV<=99 @1
C     *   Decay: x > ta+ ta- WEIGHTED<=1
C     
      IMPLICIT NONE
...
C     ----------
      ...
      ANS = 0D0
      DO IHEL=1,NCOMB
            T=MATRIX_gcx_gcx_s2(P,H1,H2 ,NHEL(1,IHEL),JC(1))
            ANS=ANS+T
      ENDDO
      ANS=ANS/DBLE(IDEN)
      END
      ...
      REAL*8 FUNCTION MATRIX_gcx_gcx_s2(P,H1,H2,NHEL,IC)
C     
C     Generated by MadGraph5_aMC@NLO v. 2.4.3, 2016-08-01
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     Amplitude(s) for diagram number 1
      ...
      CALL FFV4_1_S(W(1,2),W(1,1),GC_7,ZERO,ZERO,W(1,6))
...
C     Amplitude(s) for diagram number 4
      ...
      CALL FFV4_2_S(W(1,4),W(1,3),GC_7,ZERO,ZERO,W(1,9))
C     Amplitude(s) for diagram number 5
      CALL FFV4_0_S(W(1,9),W(1,2),W(1,6),GC_7,AMP(5))
C     Amplitude(s) for diagram number 6
      CALL VVV2_0(W(1,6),W(1,3),W(1,5),GC_6,AMP(6))
C     Amplitude(s) for diagram number 7
      CALL VVV2_0(W(1,1),W(1,8),W(1,5),GC_6,AMP(7))
      CALL VVVT2P0_1(W(1,1),W(1,3),W(1,7),GC_10,ZERO,ZERO,W(1,8))
C     Amplitude(s) for diagram number 8
      CALL FFV4_0_S(W(1,4),W(1,2),W(1,8),GC_7,AMP(8))
